...
def clip_histogram(hist, clip_limit):
    ...
    if clip_limit < 0:
        raise ValueError(
            "clip_limit must be non-negative"
        )
...
def build_tile_luts(
    ...
    if image.ndim != 2:
        raise ValueError(
            "CLAHE requires a 2D grayscale image"
        )
    ...
    row_edges = _compute_tile_edges(
        height,
        num_tiles_y
    )

    col_edges = _compute_tile_edges(
        width,
        num_tiles_x
    )
    ...
    for tile_y in range(num_tiles_y):
        ...
        for tile_x in range(num_tiles_x):
            ...
            top = row_edges[tile_y]
            bottom = row_edges[tile_y + 1]

            left = col_edges[tile_x]
            right = col_edges[tile_x + 1]
